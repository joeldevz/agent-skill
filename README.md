...
npx skill-cli-tool sync --editors cursor,antigravity
```
...
# El binario estará en: target/release/skill-cli
```
...
4. **Crea el Release en GitHub:**
   ...
   - **Sube los binarios compilados:**
     - `target/release/skill-cli` → Renombrar a `skill-cli-linux`
     - Para Windows: `target/release/skill-cli.exe` → Renombrar a `skill-cli-win.exe`
     - Para macOS: compilar en Mac y renombrar a `skill-cli-macos`
   - Publica la Release
